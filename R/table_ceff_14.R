...
mycolnames <- colnames(ert_2_14_1)

ert_2_14_1_s <- ert_2_14_1 %>% select(!c(1:5)) 
nacheck <- if_else(ert_2_14_1_s[4,1] == 'N/A', 1, 0)

if (nacheck == 1) {
ert_2_14_1_s <- ert_2_14_1_s %>% 
  mutate_all(~ str_replace(., "N/A", "0"))  %>% 
  mutate_all(~ as.numeric(.))
}

ert_2_14_1_t <- transpose(ert_2_14_1_s) %>% 
  mutate_all(~ as.numeric(.)) %>% 
...
ert_2_14_1_tt <- transpose(ert_2_14_1_t) %>% as_tibble() %>% 
  mutate(myrownames,
         .before = V1) 

data_for_table1 <- ert_2_14_1_tt %>% 
...
# data prep 2
myrownames <- ert_2_14_2[1]
mycolnames <- colnames(ert_2_14_1)

ert_2_14_2_s <- ert_2_14_2 %>% select(!c(1:5)) 
nacheck <- if_else(ert_2_14_2_s[4,1] == 'N/A', 1, 0)

if (nacheck == 1) {
  ert_2_14_2_s <- ert_2_14_2_s %>% 
    mutate_all(~ str_replace(., "N/A", "0"))  %>% 
    mutate_all(~ as.numeric(.))
}

ert_2_14_2_t <- transpose(ert_2_14_2_s) %>% 
  mutate_all(~ as.numeric(.)) %>% 
  mutate(across(c(1,2), ~format(round(.,0), big.mark = ",", scientific = F))) %>%
  mutate(across(c(3,4), ~paste0(format(round(.*100,1)), "%"))) %>% 
  as_tibble() %>% 
...
ert_2_14_2_tt <- transpose(ert_2_14_2_t)  %>% 
  mutate(myrownames,
         .before = V1) 

data_for_table2 <- ert_2_14_2_tt 
